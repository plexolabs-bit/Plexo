...
const authorName = process.env.GIT_AUTHOR_NAME || 'Your Name';
const authorEmail = process.env.GIT_AUTHOR_EMAIL || 'you@example.com';
...
for (let index = 0; index < dayOffsets.length; index += 1) {
  ...
  run('git', ['add', 'docs/fake-history-log.md'], { env: { ...process.env, GIT_AUTHOR_NAME: authorName, GIT_AUTHOR_EMAIL: authorEmail, GIT_COMMITTER_NAME: authorName, GIT_COMMITTER_EMAIL: authorEmail } });
...
console.log(`Created ${dayOffsets.length} fabricated commits with dates spread across the last month.`);
